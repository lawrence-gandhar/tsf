                            ...
                            <div class="w3-col w3-padding w3-padding-16 w3-white" style="margin-bottom:40px;">
                                ...
                                <?php foreach($results as $key => $row): ?>
                                    ...
                                    <fieldset style="margin-bottom:10px; background-color:#eee">
                                        <form action="<?php echo base_url()?>admin/examinations/<?php echo $exam_id ?>/questions/save" method="post" enctype="multipart/form-data">
                                            <input type="hidden" name="ques_id" value="<?php echo $key ?>">
                                            <div class="form-group" style="display:flow-root">
                                                <label class="col-md-12 control-label" for="total">Question <?php echo $x; ?></label>
                                                <div class="col-md-12">
                                                    <textarea required name="qns_title" class="form-control input-md"><?php echo $row["qns_title"]; ?></textarea>
                                                </div>

                                                <div class="col-md-12" style="display:block;">
                                                    <?php foreach($row["img_arr"] as $_rr): ?>
                                                        <?php if($_rr["image_id"]): ?>
                                                        <div class="col-md-2" style="margin:0px 10px 10px 0px;">
                                                        <span style="background: linear-gradient(rgba(255,255,255,0), rgba(255,255,255,0)), url(<?php echo base_url().'uploads/examinations/'.$exam_id.'/'.$key.'/'.$_rr["image_path"] ?>);
                                                            background-size: cover; height: 120px; padding:10px; width:100%; margin:10px; display:block; text-align:center;" class="image_container">
                                                            <button class="btn btn-danger" type="button" id="img_delete_but_<?php echo $_rr["image_id"] ?>" style="display:none; position: relative; top:120px; margin-top:-70px;opacity: 1; z-index:9999">
                                                ...
                                                <div class="col-md-12 pull-right" style="padding-top:10px;">
                                                    <input type="file" multiple name="img<?php echo $x?>[]">
                                                </div>
                                                        ...
                                                        <?php if($gg == 1): ?>
                                                            <div class="col-md-12" style="padding-top:10px;" id="opt_div_<?php echo $x; ?>">
                                                                <div class="col-md-9" style="padding:0px;">
                                                                    <input type="text" value="<?php echo $opts["option"] ?>" class="form-control" name="option[]">
                                                                </div>
                                                                <div class="col-md-2">
                                                                    <select class="form-control" name="correct_ans[]">
                                                                        <option value="">Is correct option</option>
                                                                        <option value="1" <?php if($opts["is_correct"] == 1) echo "selected"; ?>>Yes</option>
                                                                        <option value="0" <?php if($opts["is_correct"] == 0) echo "selected"; ?>>No</option>
                                                                    </select>
                                                                </div>
                                                                <div class="col-md-1">
                                                                    <button type="button" class="btn btn-primary" onclick="add_option_row_edit(<?php echo $key ?>)">Add</button>
                                                                </div>
                                                            </div>
                                                        <?php else: ?>

                                                            <div class="col-md-12" style="padding-top:10px;" id="opt_div_<?php echo $x; ?>">
                                                                <div class="col-md-9 m_row_<?php echo $x;?> created_row_<?php echo $x; ?>_<?php echo $gg; ?>" style="padding:0px; padding-top:5px;">
                                                                    <input type="text" value="<?php echo $opts["option"] ?>" class="form-control" name="option[]">
                                                                </div>
                                                                <div class="col-md-2 m_row_<?php echo $x;?> created_row_<?php echo $x; ?>_<?php echo $gg; ?>">
                                                                    <select class="form-control" name="correct_ans[]">
                                                                        <option value="">Is correct option</option>
                                                                        <option value="1" <?php if($opts["is_correct"] == 1) echo "selected"; ?>>Yes</option>
                                                                        <option value="0" <?php if($opts["is_correct"] == 0) echo "selected"; ?>>No</option>
                                                                ...
                                                                <div class="col-md-1 m_row_<?php echo $x;?> created_row_<?php echo $x; ?>_<?php echo $gg; ?>">
                                                                    <button type="button" class="btn btn-danger" onclick="remove_option_row_delete('<?php echo $x ?>','<?php echo $gg ?>', '<?php echo $opts['id'] ?>', '<?php  echo $key ?>')">Remove</button>
                                                                </div>
                                                            </div>
                                            ...
                                            <div class="form-group" style="text-align:center; display:flow-root">
                                                <input type="submit" class="btn btn-success" value="Save" name="submit">
                                                <a href="<?php echo base_url()?>admin/examination/<?php echo $exam_id ?>/question/<?php echo $key ?>/delete" class="btn btn-danger" >Delete Question</a>
                                            </div>
                                        </form>
                                    </fieldset>
                                ...
                                <?php for($x1 = 1; $x1 <= $new_sec; $x1++): ?>
                                  <form action="<?php echo base_url()?>admin/examinations/<?php echo $exam_id ?>/questions/save" method="post" enctype="multipart/form-data">
                                    ...
                                    <fieldset style="margin-bottom:10px; background-color:#eee">
                                        ...
                                        <div class="row form-group" style="text-align:center; display:flow-root; padding-top:15px;">
                                            <input type="submit" class="btn btn-success" value="Save" name="submit">
                                            <a href="<?php echo base_url()?>admin/examination/<?php echo $exam_id ?>/question/<?php echo $key ?>/delete" class="btn btn-danger" >Delete Question</a>
                                        </div>
                                    </fieldset>
